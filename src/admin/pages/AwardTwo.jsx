...
const AwardTwo = () => {
  ...
  const handleShow = (id) => {
    ...
    setGetId(id);
    const selectedItem = awards.find((item) => item.id === id);

    if (selectedItem) {
      setFormData({
        title: selectedItem.title,
        descp: selectedItem.descp,
        year: selectedItem.year,
        cardimg: null, // Set to null to prevent pre-filling the file input
      });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://www.joyseniorsecondary.ac.in/api/auth/updateAwardsId/${getId}`,
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(
        `https://www.joyseniorsecondary.ac.in/api/auth/deleteAwardsAchiv/${id}`
      );
  ...
  const handleUploadImages = (id) => {
    console.log(id);
    navigate(`/upload-Award-images/${id}`);
  };
            ...
            <div className="col-lg-12 col-md-12 col-md-12 col-12">
              <table className="table">
                <thead className="table-dark">
                  <tr>
                    <th className="text-center">Awards & Achievement Names</th>
