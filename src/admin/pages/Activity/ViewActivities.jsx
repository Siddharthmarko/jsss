...
const ViewActivities = () => {
  ...
  const getImage = async () => {
    try {
      const res = await axios.get( 
        `https://joyseniorsecondary.ac.in/api/auth/getactivityId/${id}`
      );
  ...
  const getBulkPicture = async () => {
    try {
      const response = await axios.get(
        "https://joyseniorsecondary.ac.in/api/auth/getActivityBulk"
      );
  ...
  const filteredImages = getBulkData.filter((image) => {
    return allImg.map((item) => item.id).includes(image.activity_id);
  });
  ...
  //   try {
  //     const deleteRes = await axios.delete(`http://localhost:4000/api/auth/deleteActivityBulkImg/${id}`);
