...
const ViewPhotoGallery = () => {
  ...
  const getImage = async () => {
    try {
      const res = await axios.get(
        `https://www.joyseniorsecondary.ac.in/api/auth/getphotoId/${id}`
      );
  ...
  const getBulkPicture = async () => {
    try {
      const response = await axios.get(
        "https://www.joyseniorsecondary.ac.in/api/auth/getPhotoBulk"
      );
  ...
  const filteredImages = getBulkData.filter((image) => {
    return allImg.map((item) => item.id).includes(image.photo_id);
  });
